...
const catalogs = [
  { id: 'latest', name: 'Últimos lançamentos', type: 'series', kind: 'latest' },
  { id: 'acao', name: 'Ação', type: 'series', kind: 'category' },
  { id: 'shounen', name: 'Shounen', type: 'series', kind: 'category' },
  { id: 'fantasia', name: 'Fantasia', type: 'series', kind: 'category' },
  { id: 'comedia', name: 'Comédia', type: 'series', kind: 'category' },
  { id: 'aventura', name: 'Aventura', type: 'series', kind: 'category' },
  { id: 'drama', name: 'Drama', type: 'series', kind: 'category' },
  { id: 'romance', name: 'Romance', type: 'series', kind: 'category' },
  { id: 'sci-fi', name: 'Sci-Fi', type: 'series', kind: 'category' },
  { id: 'sobrenatural', name: 'Sobrenatural', type: 'series', kind: 'category' },
  { id: 'slice-of-life', name: 'Slice of Life', type: 'series', kind: 'category' },
  { id: 'misterio', name: 'Mistério', type: 'series', kind: 'category' },
  { id: 'ecchi', name: 'Ecchi', type: 'series', kind: 'category' },
  { id: 'mecha', name: 'Mecha', type: 'series', kind: 'category' },
  { id: 'dublado', name: 'Dublado', type: 'series', kind: 'category' },
  { id: 'anime-chines', name: 'Anime Chinês / Donghua', type: 'series', kind: 'category' },
];
...
async function search(query, limit = PAGE_SIZE) {
  const attempts = [
    { input: { query, limit, offset: 0 } },
    { input: { search: query, limit, offset: 0 } },
  ];
...
async function handleCatalog(res, catalogId, url) {
  const config = catalogs.find((c) => c.id === catalogId);
  if (!config) return json(res, 404, { metas: [] });
  const q = clean(url.searchParams.get('search'));
  let items = q ? await search(q) : config.kind === 'latest' ? await latest() : await category(config.id);
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (match) return await handleCatalog(res, match[1], url);
